<!-- HEADER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:F9D7FF,100:D8C5FF&height=200&section=header&text=Montaha%20Ben%20Jaballah&fontSize=42&fontColor=3A2C5A&desc=%D9%85%D9%87%D9%86%D8%AF%D8%B3%D8%A9%20%D8%AD%D9%84%D9%88%D9%84%20%D8%A7%D9%84%D8%B0%D9%83%D8%A7%D8%A1%20%D8%A7%D9%84%D8%A7%D8%B5%D8%B7%D9%86%D8%A7%D8%B9%D9%8A&descSize=20&descAlignY=62" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=900&color=C7A7F8&center=true&vCenter=true&repeat=true&width=560&lines=%D8%A3%D9%86%D8%B8%D9%85%D8%A9+LLM+%D9%85%D8%AA%D8%B9%D8%AF%D8%AF%D8%A9+%D8%A7%D9%84%D9%88%D9%83%D9%84%D8%A7%D8%A1+%D9%81%D9%8A+%D8%A7%D9%84%D8%A5%D9%86%D8%AA%D8%A7%D8%AC;%D8%AE%D8%B7%D9%88%D8%B7+RAG+%D8%B9%D9%84%D9%89+pgvector;%D9%85%D9%86%D8%B4%D9%88%D8%B1%D8%A9+%D8%B9%D9%84%D9%89+Kubernetes;3-5+%D8%A3%D9%8A%D8%A7%D9%85+%E2%86%92+%D8%AF%D9%82%D9%8A%D9%82%D8%AA%D8%A7%D9%86" alt="رسوم متحركة للكتابة" />
</p>

<p align="center">
  <a href="README.md"><img src="https://img.shields.io/badge/English-4A90E2?style=for-the-badge&logo=googletranslate&logoColor=white" height="28" alt="English" /></a>
  &nbsp;
  <a href="README_FR.md"><img src="https://img.shields.io/badge/Français-E74C3C?style=for-the-badge&logo=googletranslate&logoColor=white" height="28" alt="Français" /></a>
  &nbsp;
  <a href="README_AR.md"><img src="https://img.shields.io/badge/العربية-27AE60?style=for-the-badge&logo=googletranslate&logoColor=white" height="28" alt="العربية" /></a>
</p>

<p align="center">
  <a href="https://montaha-portfolio.vercel.app"><img src="https://img.shields.io/badge/Portfolio-D8C5FF?style=for-the-badge&logo=vercel&logoColor=3A2C5A" height="28" alt="Portfolio" /></a>
  &nbsp;
  <a href="https://www.linkedin.com/in/montaha-jaballah/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="28" alt="LinkedIn" /></a>
  &nbsp;
  <a href="mailto:montaha.benjaballah@esprit.tn"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="28" alt="Email" /></a>
</p>

---

<div dir="rtl">

## 👋 نبذة عني

أبني **أنظمة مدعومة بنماذج اللغة الكبيرة وأنشرها في بيئة الإنتاج** — لا مجرد نماذج أولية.

خلال مهمة دامت ستة أشهر في **Devoteam**، أخذت عملية الرد على طلبات العروض،
المثقلة بالوثائق، من تحديد المشكلة إلى نظام يعمل فعلياً: خط أنابيب متعدد الوكلاء
يعتمد على نماذج اللغة، واسترجاع فوق قاعدة بيانات شعاعية، وواجهات برمجية،
ونشر على Kubernetes يستخدمه الناس بالفعل.

ما يهمني: هل يصمد الاسترجاع والتقييم **أمام الاستخدام الحقيقي** لا في العرض التوضيحي.
في مشروعي الأخير عنى ذلك تدقيق نموذج التقييم الخاص بي — واكتشاف أن ثلاثة من
أبعاده الستة كانت تُرجع قيماً شبه ثابتة، أي أنها لا تقيس شيئاً.

> *المؤشر الذي ينتج قيمة معقولة ليس بالضرورة مؤشراً يقيس شيئاً.*

📍 تونس · 🌍 **منفتحة على الانتقال (الاتحاد الأوروبي / عن بُعد)** · 🗣️ العربية (اللغة الأم) · الفرنسية · الإنجليزية

</div>

---

<div dir="rtl">

## 🚀 أبرز المشاريع

</div>

<table>
  <tr>
    <td width="50%" valign="top">

### 🤖 BidOrq · مشروع التخرج
**منصة ذكاء اصطناعي متعددة الوكلاء لأتمتة طلبات العروض**

خط أنابيب **LangGraph** بثلاثة وكلاء مع حلقة تصحيح ذاتي محدودة،
**RAG** فوق **pgvector**، منشور على **K3s** بأسلوب GitOps.

`٣–٥ أيام خبير ← ~دقيقتان` · `−٤٦٪ زمن استجابة` · `١١٤ اختبار`

<sub>LangGraph · Ollama · FastAPI · pgvector · Celery · K3s · ArgoCD · Istio</sub>

  </td>
    <td width="50%" valign="top">

### 📞 MediCall AI
**مساعد صوتي للمواعيد الطبية**

بنية هجينة **FSM + LLM** — آلات الحالة تجعل الحوار متوقعاً،
ونموذج اللغة يتولى الفهم. يحدّ من الهلوسة بحكم التصميم.

[**عرض حي ←**](https://medi-call-ai.vercel.app/)

<sub>Next.js 14 · TypeScript · Groq LLaMA 3.1 70B · PostgreSQL</sub>

  </td>
  </tr>
  <tr>
    <td width="50%" valign="top">

### 🔐 ISECA
**منصة امتثال لـ Azure / M365**

تقييم آلي لوضع الأمان عبر Microsoft Graph API،
مع فحوصات مجدولة وتقارير مُولَّدة.

`٩١٪ توفير في الوقت` · `+١٨٪ في درجة الأمان`

<sub>React · ASP.NET Core · PostgreSQL · Graph API · Jenkins</sub>

  </td>
    <td width="50%" valign="top">

### 🧠 نظام الأنطولوجيا الطبية
**استدلال ويب دلالي مدعوم بالذكاء الاصطناعي**

تحويل اللغة الطبيعية إلى **SPARQL** فوق أنطولوجيا OWL،
مُقدَّمة من Apache Fuseki.

<sub>React · Flask · Apache Fuseki · Ollama · SPARQL · OWL</sub>

  </td>
  </tr>
</table>

<p align="center">
  <a href="https://montaha-portfolio.vercel.app"><b>شاهد جميع المشاريع مع الصور والبنية المعمارية ←</b></a>
</p>

---

<div dir="rtl">

## 🛠️ التقنيات

**الذكاء الاصطناعي وتعلّم الآلة**

</div>

<img src="https://skillicons.dev/icons?i=python,pytorch,sklearn&theme=dark" height="42" />

`LangGraph` `RAG` `Ollama` `pgvector` `sentence-transformers` `spaCy` `HDBSCAN`

<div dir="rtl">

**الواجهة الخلفية**

</div>

<img src="https://skillicons.dev/icons?i=fastapi,python,nodejs,express,dotnet,spring&theme=dark" height="42" />

`Celery` `Redis` `REST` `GraphQL` `Server-Sent Events` `pytest`

<div dir="rtl">

**الواجهة الأمامية**

</div>

<img src="https://skillicons.dev/icons?i=react,nextjs,ts,tailwind,angular&theme=dark" height="42" />

<div dir="rtl">

**البيانات**

</div>

<img src="https://skillicons.dev/icons?i=postgres,mongodb,mysql,redis&theme=dark" height="42" />

`pgvector` `HNSW` `البحث بالتشابه الشعاعي` `Row-Level Security`

<div dir="rtl">

**السحابة و DevOps**

</div>

<img src="https://skillicons.dev/icons?i=kubernetes,docker,azure,aws,githubactions,jenkins,grafana,prometheus,linux&theme=dark" height="42" />

`K3s` `ArgoCD (GitOps)` `Istio (mTLS)` `Helm` `Kyverno` `Velero` `Loki` `Terraform`

---

<div dir="rtl">

## 📜 الشهادات

</div>

<p>
  <img src="https://img.shields.io/badge/Microsoft-Azure_AI_Apps_%26_Agents_Developer_Associate-0078D4?style=flat-square&logo=microsoftazure&logoColor=white" height="26" />
  <br/>
  <img src="https://img.shields.io/badge/Oracle-OCI_2025_DevOps_Professional-F80000?style=flat-square&logo=oracle&logoColor=white" height="26" />
  <br/>
  <img src="https://img.shields.io/badge/Oracle-OCI_2025_Foundations_Associate-F80000?style=flat-square&logo=oracle&logoColor=white" height="26" />
  <br/>
  <img src="https://img.shields.io/badge/AWS-Cloud_Foundations-FF9900?style=flat-square&logo=amazonwebservices&logoColor=white" height="26" />
  <br/>
  <img src="https://img.shields.io/badge/Aviatrix-Multicloud_Network_Associate-FF6B00?style=flat-square&logo=aviato&logoColor=white" height="26" />
  <br/>
  <img src="https://img.shields.io/badge/NVIDIA-NLP_Transformers_·_Deep_Learning_×2-76B900?style=flat-square&logo=nvidia&logoColor=white" height="26" />
</p>

---

<div dir="rtl">

## 📊 إحصائيات GitHub

</div>

<p align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=MontahaJaballah&show_icons=true&theme=midnight-purple&hide_border=true&icon_color=C7A7F8&title_color=D8C5FF&text_color=EDE7F6&bg_color=00000000" height="165" alt="إحصائيات GitHub" />
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=MontahaJaballah&layout=compact&theme=midnight-purple&hide_border=true&title_color=D8C5FF&text_color=EDE7F6&bg_color=00000000&langs_count=8" height="165" alt="اللغات الأكثر استخداماً" />
</p>

---

<p align="center">
  <b>منفتحة على وظائف الذكاء الاصطناعي وهندسة المنصات — أوروبا، عن بُعد، أو هجين.</b>
  <br/><br/>
  <a href="mailto:montaha.benjaballah@esprit.tn">montaha.benjaballah@esprit.tn</a>
  &nbsp;·&nbsp;
  <a href="https://montaha-portfolio.vercel.app">Portfolio</a>
  &nbsp;·&nbsp;
  <a href="https://www.linkedin.com/in/montaha-jaballah/">LinkedIn</a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=MontahaJaballah&color=D8C5FF&style=flat-square&label=Profile+views" alt="مشاهدات الملف الشخصي" />
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:D8C5FF,100:F9D7FF&height=140&section=footer" />
</p>
